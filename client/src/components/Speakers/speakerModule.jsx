...
export default function SpeakerModule() {
  ...
  const speakerChunks = chunkArray(activeSpeakers, chunkSize);
            ...
            <div key={index} className="flex justify-center space-x-4 px-2">
              {chunk.map((speaker) => (
                <div key={speaker.id} className="flex flex-col p-4 rounded-lg shadow-md bg-white w-full max-w-[300px]">
                  <img
                    src={speaker.Image || 'https://w7.pngwing.com/pngs/340/946/png-transparent-avatar-user-computer-icons-software-developer-avatar-child-face-heroes-thumbnail.png'}
                  ...
                  <div className='h-[100px]'>
                    <h3 className="text-sm sm:text-md lg:text-lg font-semibold text-start">{speaker.Nombre || 'Nombre'} {speaker.Apellido || 'Apellido'}</h3>
                    <p className="text-sm sm:text-sm text-gray-600 text-start">{speaker.Empresa || 'Empresa'}</p>
                    <p className="text-md sm:text-md text-gray-600 font-bold text-start">{speaker.Cargo || 'Cargo'}</p>
                    <div className='flex justify-between items-center w-full mt-2'>
                      <div className="flex items-center space-x-2">
                        {countryFlags[speaker.Pais] && (
                          <img 
                            src={countryFlags[speaker.Pais]} 
                            alt={`Bandera de ${speaker.Pais}`} 
                            className="h-4 w-6"
                          />
                        )}
