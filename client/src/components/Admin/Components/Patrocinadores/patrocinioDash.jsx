...
export default function PatrocinioDash() {
  ...
  const filteredSponsoreData = Object.keys(sponsoreData).reduce((acc, category) => {
    const filteredSponsors = sponsoreData[category].filter((sponsor) =>
      sponsor.Url.toLowerCase().includes(searchQuery.toLowerCase())
    );
    if (filteredSponsors.length > 0) {
      acc[category] = filteredSponsors;
    }
    return acc;
  }, {});
  ...
  return (
    <div className="p-8 ml-12">
      ...
      <div className="sponsore-list">
        {Object.keys(filteredSponsoreData).map((category) => (
          <div key={category} className="overflow-x-auto mb-8">
            <table className="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredSponsoreData[category].map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="whitespace-nowrap px-4 py-2 font-medium text-gray-900">{item.Type}</td>
                    <td className="whitespace-nowrap px-4 py-2 text-gray-700">
                      {editMode === item.id ? (
                        <select
